...
object GeneradorBoletaPdf {
    ...
    fun generar(
        ...
        try {
            ...
            qrCambio(doc, venta, configuracion)
        } finally {
            doc.close()
        }
    }
    ...
    private fun encabezado(doc: Document, configuracion: ConfiguracionTiendaEntity?) {
        ...
        // Descripción opcional de la tienda: solo se agrega si el dueño la configuró.
        // Margen chico para que quede pegada al eslogan, como un solo bloque de texto.
    ...
    private fun etiquetaComprobante(venta: VentaEntity): String = when (venta.tipoComprobante) {
        ...
        // decir "Boleta" ni "Factura" en ningún lado del PDF.
    ...
    private fun datosVenta(doc: Document, venta: VentaEntity) {
        ...
        tabla.addCell(
            Cell().add(Paragraph("${etiquetaComprobante(venta)} N° $folioTexto").setBold().setFontSize(13f))
                .setBorder(null)
        )
        tabla.addCell(
        ...
        // Datos del cliente, si se capturaron: DNI/nombre siempre opcionales;
        // RUC/razón social solo aplican cuando el comprobante es factura.
        val esFactura = venta.tipoComprobante == com.tuempresa.possystem.data.local.entity.TipoComprobante.FACTURA
        if (esFactura && (!venta.clienteRuc.isNullOrBlank() || !venta.clienteRazonSocial.isNullOrBlank())) {
            venta.clienteRazonSocial?.takeIf { it.isNotBlank() }?.let {
                doc.add(Paragraph("Cliente: $it").setFontSize(10f))
            }
            venta.clienteRuc?.takeIf { it.isNotBlank() }?.let {
                doc.add(Paragraph("RUC: $it").setFontSize(10f))
            }
        } else if (!venta.clienteNombre.isNullOrBlank() || !venta.clienteDocumento.isNullOrBlank()) {
            venta.clienteNombre?.takeIf { it.isNotBlank() }?.let {
                doc.add(Paragraph("Cliente: $it").setFontSize(10f))
            }
            venta.clienteDocumento?.takeIf { it.isNotBlank() }?.let {
                doc.add(Paragraph("DNI: $it").setFontSize(10f))
            }
        }
    ...
    /**
     * QR pequeño que codifica el ID de la venta, para que al hacer un cambio
     * el vendedor solo escanee la boleta en vez de buscar el folio a mano.
     */
    private fun qrCambio(doc: Document, venta: VentaEntity, configuracion: ConfiguracionTiendaEntity?) {
        val bitmap = GeneradorQr.generar("$PREFIJO_QR_VENTA${venta.id}", tamanoPx = 200) ?: return
        val bytes = bitmapABytesPng(bitmap) ?: return
        try {
            val qr = Image(ImageDataFactory.create(bytes))
            // Pequeño y a la esquina inferior derecha: es un QR de uso interno
            // (vendedor escaneando una devolución), no algo que el cliente
            // necesite ver destacado como el de redes sociales.
            qr.scaleToFit(32f, 32f)
            qr.setHorizontalAlignment(HorizontalAlignment.LEFT)
            qr.setMarginTop(18f)
            doc.add(qr)
            doc.add(
                Paragraph("Escanea para cambios/devoluciones")
                    .setFontSize(6.5f)
                    .setFontColor(ColorConstants.GRAY)
            ...
            // Si el QR no se puede insertar, la boleta se sigue generando sin él.
        }
        ...
        // Va después de todo el bloque de redes/pie de página.
        configuracion?.politicaCambios?.takeIf { it.isNotBlank() }?.let {
            doc.add(
                Paragraph("⚠️ $it")
                    .setFontSize(8f)
                    .setFontColor(DeviceRgb(0x8A, 0x4B, 0x3A))
                    .setTextAlignment(TextAlignment.CENTER)
                    .setMarginTop(10f)
            )
        }
    ...
    private fun piePagina(doc: Document, configuracion: ConfiguracionTiendaEntity?) {
        ...
        val linkRedes = configuracion?.linkRedesSociales?.takeIf { it.isNotBlank() } ?: return
        val bitmap = GeneradorQr.generar(linkRedes, tamanoPx = 200) ?: return
        val bytes = bitmapABytesPng(bitmap) ?: return
        try {
            val qr = Image(ImageDataFactory.create(bytes))
            ...
            doc.add(qr)
            doc.add(
                Paragraph("Síguenos en nuestras redes")
                    .setFontSize(9f)
                    .setFontColor(ColorConstants.GRAY)
                    .setTextAlignment(TextAlignment.CENTER)
                    .setMarginTop(2f)
            ...
            // Si el QR de redes no se puede insertar, la boleta se sigue generando sin él.
        }
    }

    private fun bitmapABytesPng(bitmap: android.graphics.Bitmap): ByteArray? {
        return try {
            val salida = java.io.ByteArrayOutputStream()
            bitmap.compress(android.graphics.Bitmap.CompressFormat.PNG, 100, salida)
            salida.toByteArray()
        } catch (e: Exception) {
            null
        }

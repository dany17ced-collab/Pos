...
class GeneradorDatosReporte(private val app: POSApplication) {
    ...
    /**
     * Genera el reporte para SEMANAL/MENSUAL (calculados desde `referencia`) o
     * para PERSONALIZADO, en cuyo caso `desdePersonalizado`/`hastaPersonalizado`
     * son obligatorios y delimitan el rango exacto elegido por el usuario.
     */
    suspend fun generar(
        periodo: PeriodoReporte,
        referencia: Long = System.currentTimeMillis(),
        desdePersonalizado: Long? = null,
        hastaPersonalizado: Long? = null
    ): DatosReporte {
        val (desde, hasta) = if (periodo == PeriodoReporte.PERSONALIZADO) {
            require(desdePersonalizado != null && hastaPersonalizado != null) {
                "PERSONALIZADO requiere desdePersonalizado y hastaPersonalizado"
            }
            desdePersonalizado to finDelDia(hastaPersonalizado)
        } else {
            rangoParaPeriodo(periodo, referencia)
        }
        ...
        val ventas = ventaDao.obtenerVentasEnRango(tiendaId, desde, hasta)
        ...
        // Se excluyen los registros de APERTURA: son solo el fondo inicial declarado
        // al empezar el turno, no un corte de rendición de cuentas (X/Z).
        val cortes = corteCajaDao.obtenerHistorialEnRango(tiendaId, desde, hasta)
            .filter { it.tipo != com.tuempresa.possystem.data.local.entity.TipoCorte.APERTURA }
    ...
    /** Rango para SEMANAL/MENSUAL. PERSONALIZADO nunca llega aquí (se resuelve antes, en generar()). */
    private fun rangoParaPeriodo(periodo: PeriodoReporte, referencia: Long): Pair<Long, Long> {
        ...
        when (periodo) {
            ...
            PeriodoReporte.PERSONALIZADO -> error("PERSONALIZADO se resuelve en generar(), no aquí")
        }
    ...
    /** Epoch millis de las 23:59:59.999 del día que cae `fecha`, para incluir todo el último día de un rango. */

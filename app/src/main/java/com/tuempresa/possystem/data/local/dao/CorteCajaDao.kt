...
interface CorteCajaDao {
    ...
    @Query("SELECT * FROM cortes_caja WHERE tiendaId = :tiendaId ORDER BY fechaCorte DESC")
    fun observarHistorial(tiendaId: String): Flow<List<CorteCajaEntity>>

    /** Igual que observarHistorial, pero de un solo disparo — usada para generar reportes de una tienda. */
    @Query("SELECT * FROM cortes_caja WHERE tiendaId = :tiendaId AND fechaCorte BETWEEN :desde AND :hasta ORDER BY fechaCorte DESC")
    suspend fun obtenerHistorialEnRango(tiendaId: String, desde: Long, hasta: Long): List<CorteCajaEntity>

    /** Cortes de TODAS las tiendas en un rango — usado en el reporte consolidado del administrador. */
    @Query("SELECT * FROM cortes_caja WHERE fechaCorte BETWEEN :desde AND :hasta ORDER BY fechaCorte DESC")
    suspend fun obtenerHistorialEnRangoGlobal(desde: Long, hasta: Long): List<CorteCajaEntity>

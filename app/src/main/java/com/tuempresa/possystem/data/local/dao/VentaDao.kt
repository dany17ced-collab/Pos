...
abstract class VentaDao {
    ...
    /**
     * Correlativo numérico para el folio: MAX(folio)+1, empezando en 1,
     * calculado POR TIENDA y POR TIPO DE SERIE — como exige la normativa de
     * comprobantes por establecimiento. BOLETA y FACTURA comparten la MISMA
     * serie tributaria (esNotaVenta = false) porque así es como se configura
     * normalmente el punto de venta ante SUNAT; NOTA_VENTA lleva su propio
     * correlativo interno separado (esNotaVenta = true), para que emitir notas
     * de venta nunca deje huecos en la numeración tributaria de boletas/facturas.
     */
    @Query(
        """
        SELECT COALESCE(MAX(folio), 0) FROM ventas 
        WHERE tiendaId = :tiendaId 
        AND (tipoComprobante = 'NOTA_VENTA') = :esNotaVenta
        """
    )
    protected abstract suspend fun obtenerUltimoFolio(tiendaId: String, esNotaVenta: Boolean): Long
    ...
    open suspend fun registrarVentaCompleta(
        ...
        // Folio 100% numérico y correlativo por tienda (nunca el UUID de la venta),
        // asignado aquí dentro de la transacción para que dos ventas de la misma
        // sucursal nunca puedan quedar con el mismo número aunque se registren
        // casi al mismo tiempo. Ver obtenerUltimoFolio: notas de venta llevan su
        // propio correlativo, separado del de boletas/facturas.
        val esNotaVenta = venta.tipoComprobante == TipoComprobante.NOTA_VENTA
        val siguienteFolio = obtenerUltimoFolio(venta.tiendaId, esNotaVenta) + 1
        insertarVenta(venta.copy(folio = siguienteFolio))
    ...
    /** Búsqueda manual por folio — respaldo cuando el cliente no trae la boleta física para escanear. */
    @Query("SELECT * FROM ventas WHERE folio = :folio LIMIT 1")
    abstract suspend fun obtenerPorFolio(folio: Long): VentaEntity?

    @Query("SELECT * FROM ventas WHERE tiendaId = :tiendaId AND fecha BETWEEN :desde AND :hasta ORDER BY fecha DESC")
    abstract fun observarVentasEnRango(tiendaId: String, desde: Long, hasta: Long): Flow<List<VentaEntity>>

    /** Ventas de un usuario específico en una tienda y rango de fechas — usada en "Mis ventas de hoy". */
    @Query(
        """
        SELECT * FROM ventas 
        WHERE tiendaId = :tiendaId AND usuarioId = :usuarioId AND fecha BETWEEN :desde AND :hasta 
        ORDER BY fecha DESC
        """
    )
    abstract fun observarVentasDeUsuarioEnRango(
        tiendaId: String,
        usuarioId: String,
        desde: Long,
        hasta: Long
    ...
    /** Igual que observarVentasEnRango, pero de un solo disparo — usada para generar reportes de una tienda. */
    @Query("SELECT * FROM ventas WHERE tiendaId = :tiendaId AND fecha BETWEEN :desde AND :hasta ORDER BY fecha DESC")
    abstract suspend fun obtenerVentasEnRango(tiendaId: String, desde: Long, hasta: Long): List<VentaEntity>

    /** Ventas de TODAS las tiendas en un rango — usada en el dashboard consolidado del administrador. */
    @Query("SELECT * FROM ventas WHERE fecha BETWEEN :desde AND :hasta ORDER BY fecha DESC")
    abstract suspend fun obtenerVentasEnRangoGlobal(desde: Long, hasta: Long): List<VentaEntity>

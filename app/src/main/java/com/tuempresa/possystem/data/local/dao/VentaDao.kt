...
abstract class VentaDao {
    ...
    /** Correlativo numérico para el folio de boleta: MAX(folio)+1, empezando en 1. */
    @Query("SELECT COALESCE(MAX(folio), 0) FROM ventas")
    protected abstract suspend fun obtenerUltimoFolio(): Long
    ...
    open suspend fun registrarVentaCompleta(
        ...
        // Folio 100% numérico y correlativo (nunca el UUID de la venta), asignado
        // aquí dentro de la transacción para que dos ventas nunca puedan quedar
        // con el mismo número aunque se registren casi al mismo tiempo.
        val siguienteFolio = obtenerUltimoFolio() + 1
